...
@threestudio.register("diffuse-with-point-light-material-progressive3d")
        ...
        **kwargs,
        ...
        if ambient_ratio is not None:
            ...
            diffuse_light_color = torch.full_like(
                self.diffuse_light_color, self.random_num
            )
        ...
        if shading is None:
            if self.training:
                ...
                if self.ambient_only or self.random_num > self.cfg.diffuse_prob:
                    shading = "albedo"
                elif self.random_num < self.cfg.textureless_prob:
                    shading = "textureless"
                else:
                    shading = "diffuse"
            else:
